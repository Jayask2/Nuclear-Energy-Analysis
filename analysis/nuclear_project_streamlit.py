...
df_nrg_consumption=pd.read_csv('share-energy-consum-by-source.csv')
...
### **Cleaning the global_power_plant_database_last.csv**
df_global_power_plants['estimated_generation_note_2013'].unique()
...
df_global_power_plants['commissioning_year'].unique()

### **Create a second dataframe based in commissioning year**
df_global_2=df_global_power_plants.copy()
df_global_2.dropna(subset=['commissioning_year'],inplace=True)
df_global_2['commissioning_year']=df_global_2['commissioning_year'].astype(int)
df_global_power_plants.drop(columns=['source','url','geolocation_source'],inplace=True)
df_global_2.drop(columns=['source','url','geolocation_source'],inplace=True)
df_global_power_plants.drop(columns=['other_fuel2','other_fuel3'],inplace=True)
df_global_2.drop(columns=['other_fuel2','other_fuel3'],inplace=True)
df_global_power_plants.drop(columns=['wepp_id'],inplace=True)
df_global_2.drop(columns=['wepp_id'],inplace=True)
df_global_power_plants.drop(columns=['generation_data_source'],inplace=True)
df_global_2.drop(columns=['generation_data_source'],inplace=True)

### **Cleaning the other dataframes**
...
df_nrg_consumption['Oil (% sub energy)']=df_nrg_consumption['Oil (% sub energy)'].round(2)
df_nrg_consumption['Coal (% sub energy)']=df_nrg_consumption['Coal (% sub energy)'].round(2)
df_nrg_consumption['Solar (% sub energy)']=df_nrg_consumption['Solar (% sub energy)'].round(2)
df_nrg_consumption['Nuclear (% sub energy)']=df_nrg_consumption['Nuclear (% sub energy)'].round(2)
df_nrg_consumption['Hydro (% sub energy)']=df_nrg_consumption['Hydro (% sub energy)'].round(2)
df_nrg_consumption['Wind (% sub energy)']=df_nrg_consumption['Wind (% sub energy)'].round(2)
df_nrg_consumption['Gas (% sub energy)']=df_nrg_consumption['Gas (% sub energy)'].round(2)
df_nrg_consumption['Other renewables (% sub energy)']=df_nrg_consumption['Other renewables (% sub energy)'].round(2)

### **Create a new column with the continents in dataframe of power plants**
regions = {
    "Asia": [
...
### Categorize the energy sources in three different categories
...
### **Create a column in dataframe of energy consumption which specifies the continent**
...
### Electricity production dataframe
...
### **Create a column in dataframe of energy production which specifies the continent**

df_elec_production=df_elec_production[(df_elec_production['Entity']=='Europe') | (df_elec_production['Entity']=='North America') | (df_elec_production['Entity']=='Africa') | (df_elec_production['Entity']=='Australia') | (df_elec_production['Entity']=='South & Central America') | (df_elec_production['Entity']=='World') | (df_elec_production['Entity']=='Asia Pacific')]

df_elec_production['Entity'] = df_elec_production['Entity'].replace({
    'Australia': 'Oceania',
    'South & Central America': 'South America',
    'Asia Pacific': 'Asia'
})

### **Change CO2 emissions Cvs to millions**

df_emissions['Annual CO₂ emissions'] = (
    df_emissions['Annual CO₂ emissions'] / 1000000
).round(2)
...
if page=="Fuel Trends in Energy Consumption and Energy Production":
    ...
    filtered_nrg_consumption = filter_by_continent(df_nrg_consumption, Continent)
